...
// pages/my/expense-card/expense-card.js
Page({
  ...
  onLoad: function(options) {
    this.url = app.globalData.urlBase + '/spread/mp/mine/queryMineConsumerCard';
    util.http(this.url, {}).then(this.processExpenseCardData);
  },
          ...
          wx.showModal({
            ...
            success(res) {
              let index = _this.currentItemIndex,
                listData = _this.data.expenseCardData;
              if (listData[index].cardType === '1') {
                listData[index].remainTimes = listData[index].remainTimes - 1;
                if (listData[index].remainTimes < 1) {
                  listData[index].status = '2'
                }
              }
              _this.setData({
                expenseCardData: listData,
                isShowQrCode: false
              })
            }
          })
  ...
  onReachBottom: function() {
    if (this.data.isHaveMoreData) {
      ...
      util.http(this.url, param).then((data) => {
        ...
        this.setData({
          expenseCardData: this.data.expenseCardData.concat(data.results),
          isLoadingMore: false,
          pageData: data.data
        });
      });
  ...
  processExpenseCardData: function(data) {
