// pages/recordedLesson/recruit/recruit.js
...
Page({
        ...
        success: (result) => {
          if (res.errorCode == 9000) {
            if (this.pages.length == 1) {
              wx.switchTab({
                url: '/pages/tabBar/recordedLesson/index'
              })
            } else {
              wx.navigateBack({
                delta: 1
  ...
  handleClick() {
    if (this.agree === '1') {
      ...
      wx.showModal({
        title: '提示',
        showCancel: false,
        content: '需要勾选《闪闪课堂推广协议》才能成为推广达人',
        confirmText: '确定',
        confirmColor: '#3CC51F',
        success: (result) => {
  ...
  navTo(e) {
    wx.navigateTo({
      url: '/pages/recordedLesson/promotion/promotion',
    });
  },
  navToPoster() {
    wx.navigateTo({
      url: '/pages/recordedLesson/recruit-poster/recruit-poster',
    });
  },
  /**
   * 页面相关事件处理函数--监听用户下拉动作
